      ...
      <b-row>
          <b-col cols="3" :key="card.title" class="DC-card" v-for="card in cards">
            <b-card :header="card.title | toUpperCase"  align="center">
              <div class="overlay">
                  <div class="addToDeck">
                    <b-button @click="addCard(card)">Ajouter au deck</b-button>
                  </div>
              </div>
              <b-card-text>COÛT : {{ card.cost }}</b-card-text>
               <b-card-footer>
                 <div class="stats atk">ATK : <span>{{ card.attack }}</span></div>
                 <div class="stats def">DEF : <span>{{ card.defense }}</span></div>
               </b-card-footer>
            </b-card>
          </b-col>
      </b-row>
